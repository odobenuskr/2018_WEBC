...
(function (root, factory) {
    ...
    String.prototype.toDate2 = function () {
        var dt = new Date();
        var dot = this.replace(/\d/g, '').charAt(0);
        var arr = this.split(dot);
...
// YMK: replace the following line to the next
...
day = day2.toDate();
var f = _this.data.find(function(element) {
if(isDate(element.date))
  return element.date.isSame(day);
else
  return element.date.toDate().isSame(day);
});
var v = (f) ? $(this).data(MARK_DATA) : 'no value';
var id = (f) ? f.id : null;
console.log(f);
//var r = _this.options.onSelected.call(this, 'date', day2, v, id);
var r = _this.options.onSelected.call(this, 'date', day2, f, id);
/*
if(r != null)
{
$(this).data(MARK_DATA, r);
//var sDate = date.toDate();
  _this.data.find(function(element) {
    if(isDate(element.date))
      return element.date.isSame(day);
    else
      return element.date.toDate().isSame(day);
...
// YMK: add next three methods
        addSchedule: function (schedule) {
            this.data.push(schedule);

            var arr = this.getDisDateValue();
            vc = this.options.viewChange;

            this.updateMonthView(arr[0], arr[1]);

            vc('month', arr[0], arr[1]);

            this.updateDateView(arr[0], arr[1]);
            vc('date', arr[0]);
        },

        syncSchedule: function (data) {
            this.data = data;
        ...
        editSchedule: function (day, r) {
          var day2 = day.toDate2();
          day = day2.toDate();
        var f = this.data.find(function(element) {
          if(isDate(element.date))
            return element.date.isSame(day);
          else
            return element.date.toDate().isSame(day);
        ...
        deleteSchedule: function (date) {
          var sDate;
          if(date != null) sDate = date.toDate();
//          else sDate = ;
            this.data.splice(this.data.findIndex(function(element) {
              if(isDate(element.date))
                return element.date.isSame(sDate);
              else
                return element.date.toDate().isSame(sDate);
              }), 1);
